...
export async function getUserIPs() {
  ...
  try {
    ...
    console.error('無法從 ipify 獲取 IP (2):', error);
  }
...
function isAnyIPInWhitelist(userIPs, whitelist) {
  ...
  return userIPs.some(userIP => {
    ...
    if (whitelist.includes(userIP)) {
      console.log(`IP ${userIP} 完全匹配白名單 (2)`);
      return true;
    }
    ...
    const prefixMatch = whitelist.some(whitelistedIP => {
      ...
      if (whitelistedIP.split('.').length < 4) {
        ...
        if (isMatch) {
          console.log(`IP ${userIP} 前綴匹配白名單項 ${whitelistedIP} (2)`);
        }
        return isMatch;
      }
      return false;
...
export async function handleCheckin(device, type, name, location, lang, uid, statusElement) {
  ...
  try {
    await addDoc(collection(window.db, '2checkins'), {
      ...
      device: device,
      userId: uid,
      ips: userIPs // 儲存所有檢測到的 IP，方便後續分析查詢
    });
